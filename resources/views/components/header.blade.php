{{-- header --}}
<header class="container {{ request()->is('order-now/*') ? 'hidde p-2 px-6 lg:px-12' : ' p-2 lg:px-6' }} flex justify-between items-center font-semibold">
    ...
    {{-- mobile screen --}}
    <div class="flex items-end justify-center gap-5 lg:items-center">
            ...
            <div class="relative profile-dropdown-wrapper">
                ...
                <div class="dropdown-profile-menu hidden absolute -bottom-[6.3rem] right-0 z-10 bg-white rounded-xl rounded-se-none shadow-lg border-4 border-slate-100 text-primary font-medium text-sm w-max overflow-hidden duration-300">
                    <div class="flex flex-col items-start text-xs font-medium helper-flex-display ">
                        <a href="{{ route('customer.dashboard') }}" class="py-3 rounded-lg ps-4 pe-12 hover:bg-slate-100">Akun saya</a>
                        <button id="logoutBtn" class="w-full py-3 text-red-400 duration-150 rounded-lg pe-12 hover:bg-red-500 active:bg-red-600 hover:text-white">Logout</button>
                    </div>
                </div>
            </div>
            @else
            <a href="{{ route('login') }}">
                <button id="login-btn" class="px-4 py-3 text-xs font-medium text-white duration-150 rounded-full bg-primary-700 hover:bg-primaryHovered active:bg-primary-600 lg:text-xs">Login</button>
            </a>
            @endauth
    ...
    <nav class="mobile-nav hidden absolute -top-[100vh] left-0 w-full h-full pt-14 z-20 transition-all duration-700 ease-in-out bg-white overflow-hidden">
        <iconify-icon icon="material-symbols:close" id="hide-menu-btn" class="absolute p-3 text-2xl duration-100 cursor-pointer top-3 right-3 text-secondary hover:text-primary active:scale-75 active:text-secondary"></iconify-icon>
        <ul class="flex flex-col gap-6 text-sm font-normal text-center text-secondary">
