...
@section('content')
    ...
    <section id="hero-section" class="container px-4 relative text-white">
        <div
            class="img-overlay-group container w-full h-[24rem] lg:h-[36rem] overflow-hidden relative rounded-xl lg:rounded-3xl">
            ...
            <div class="overlay w-full h-full bg-black/30 absolute top-1/2 left-1/2  -translate-x-1/4 -translate-y-1/4">
            </div>
        </div>
        <div
            class="text-button-group absolute top-1/2 left-1/2 -translate-x-1/4 -translate-y-1/4 flex flex-col items-center content-center gap-4 text-center">
            <h1 class="text-5xl lg:text-7xl font-semibold w-max">Buat hidangan acaramu<br>jadi lebih berkualitas.</h1>
            <p class="text-sm lg:text-lg -mt-2 lg:mt-2">Katering Ibu - Solusi Kebutuhan Katering Anda.</p>
            @auth
                <a href="{{ route('menu') }}">
                    <button id="btn-order-now"
                        class="group ps-2 pe-4 py-2 lg:pe-5 rounded-full bg-primary hover:bg-gradient-to-r hover:from-primaryHovered hover:to-primary hover:scale-[1.01] duration-200 text-xs lg:text-sm text-white/60 hover:text-white active:bg-primary flex items-center content-center gap-2 lg:gap-3 mt-3">
                        <iconify-icon icon="akar-icons:shopping-bag"
                            class="text-lg lg:text-2xl p-2 bg-primary-600 rounded-full group-hover:bg-primary duration-200"></iconify-icon>
                        {{ auth()->check() ? 'Pesan sekarang' : 'Buat Pesanan Pertama Anda' }}
                        <iconify-icon icon="fluent:arrow-right-20-filled"
                            class="text-lg lg:text-xl group-hover:ms-6 duration-200"></iconify-icon>
                    </button>
                </a>
            @else
                <a href="{{ route('login') }}">
                    <button id="btn-order-now"
                        class="group ps-2 pe-4 py-2 rounded-full bg-primary hover:bg-gradient-to-r hover:from-primaryHovered hover:to-primary duration-200 text-xs lg:text-sm text-secondary hover:text-white active:bg-primary flex items-center content-center gap-2 mt-3">
                        <iconify-icon icon="akar-icons:shopping-bag"
                            class="text-lg p-2 bg-primary-600 rounded-full group-hover:bg-primary duration-200"></iconify-icon>
                        Ayo mulai memesan
                        <iconify-icon icon="fluent:arrow-right-20-filled"
                            class="text-lg group-hover:ms-6 duration-200"></iconify-icon>
                    </button>
                </a>
            @endauth
        </div>
    ...
    <section id="features-section" class="container flex items-center justify-center lg:h-[26rem] px-4 mt-6">
        <div class="features-wrapper container bg-white lg:bg-transparent px-6 pt-6 lg:pt-20 rounded-xl cursor-default">
            ...
            <div
                class="content-wrapper text-center lg:text-left text-sm text-primary hover:text-secondary flex gap-2 lg:gap-5 py-10">
                <div
                    class="feature-item lg:bg-slate-100 lg:p-2 lg:rounded-xl flex flex-col lg:flex-row items-center content-center gap-2 lg:gap-4 flex-1 lg:text-base hover:text-primary duration-150">
                ...
                <div
                    ...
                    <iconify-icon icon="medical-icon:i-social-services"
                        class="text-5xl lg:p-5 lg:rounded-xl lg:bg-white"></iconify-icon>
                    <h2 class="line-clamp-2 lg:line-clamp-none overflow-hidden">Layanan profesional dengan penuh kepercayaan
                    </h2>
                </div>
                <div
                    class="feature-item lg:bg-slate-100 lg:p-2 lg:rounded-xl flex flex-col lg:flex-row items-center content-center gap-2 lg:gap-4 flex-1 lg:text-base hover:text-primary duration-150">
                    <iconify-icon icon="ri:service-fill" class="text-5xl lg:p-5 lg:rounded-xl lg:bg-white"></iconify-icon>
                    <h2 class="line-clamp-2 lg:line-clamp-none overflow-hidden">Dipercaya oleh lebih dari 100+ pelanggan
                        setia</h2>
                </div>
                <div
                    class="feature-item lg:bg-slate-100 lg:p-2 lg:rounded-xl flex flex-col lg:flex-row items-center content-center gap-2 lg:gap-4 flex-1 lg:text-base hover:text-primary duration-150">
                    <iconify-icon icon="healthicons:money-bag"
                        class="text-5xl lg:p-5 lg:rounded-xl lg:bg-white"></iconify-icon>
                    <h2 class="line-clamp-2 lg:line-clamp-none overflow-hidden">Harga kompetitif dengan cita rasa istimewa
                    </h2>
                </div>
            </div>
        </div>
    ...
    <section id="top-menu-section" class="container px-4">
        <div class="top-menu-wrapper w-full p-6 bg-white md:bg-transparent mt-6 rounded-xl">
            ...
            <div class="head-content hidden lg:flex text-center flex-col gap-4 mb-3">
            ...
            <div
                class="card-wrapper group grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-6 text-primary hover:text-secondary ">
                @foreach ($bestSellingMenus as $item)
                    <figure
                        class="card relative flex-1 hover:text-primary duration-150 border border-transparent p-3 rounded-2xl hover:border-slate-300">
                        ...
                        <figcaption class="card-content mt-4 flex flex-col gap-1">
                            <div class="text-wrapper relative w-full flex gap-3 items-start justify-between">
                                <div class="flex flex-col gap-2 text-primary">
                                    ...
                                    <p class="menu-price font-bold text-lg before:content-['Rp']">
                                        {{ number_format($item->harga, 0, ',', '.') }}</p>
                                </div>
                                {{-- <span
                                    class="label_terjual min-w-max h-max text-[.65rem] px-2 py-1.5 -translate-y-1 rounded-full bg-blue-50 text-blue-400 select-none">Terjual
                                    {{ $item->terjual }} porsi</span> --}}
                            </div>
                            @auth
                                <a href="{{ route('order-now.show', ['order_now' => $item->id]) }}" class="w-full">
                                    <button
                                        class="btn-order w-full bg-orderDeactive hover:bg-orderHovered active:bg-orderClicked text-white mt-4 py-3 text-xs lg:text-sm flex items-center justify-center gap-1 rounded-lg duration-150">
                                        <iconify-icon icon="tdesign:shop-filled"
                                            class="text-base -translate-y-[1px]"></iconify-icon>
                                        Pesan Sekarang
                                    </button>
                                </a>
                            @else
                                <a href="{{ route('login') }}" class="w-full">
                                    <button
                                        class="btn-order w-full bg-secondary-300 hover:bg-secondary active:bg-tertairy text-white mt-4 py-3 text-xs lg:text-sm flex items-center justify-center gap-1 rounded-lg duration-150 cursor-default">
                                        <iconify-icon icon="tdesign:shop-filled"
                                            class="text-base -translate-y-[1px]"></iconify-icon>
                                        Pesan Sekarang
                                    </button>
        ...
        <div class="menu-wrapper w-full px-6 bg-white mt-6 lg:mt-16 py-6 rounded-xl">
            ...
            <div
                ...
                @foreach ($menu as $item)
                    <figure
                        class="card relative flex-1 hover:text-primary duration-150 border border-transparent p-3 pb-4 rounded-2xl hover:border-slate-300">
                        <div class="img-container aspect-4/3 rounded-2xl overflow-hidden">
                            <img src="{{ Storage::url($item->foto_menu) }}" alt="Foto {{ $item->nama_menu }}"
                                class="w-full h-full object-cover brightness-100 duration-200">
                            {{-- <div class="rating-menu absolute top-6 left-5 bg-white px-3 py-1 rounded-full flex items-center content-center gap-1 font-medium text-sm">
            ...
            <a href="{{ route('menu') }}" class="block place-self-end">
                <button id="btn-lihat-semua"
                    class="w-full bg-tertiary text-primary hover:bg-slate-200/70 active:bg-slate-300/70 duration-150 p-4 mt-5 rounded-md text-xs lg:text-sm lg:font-medium lg:w-max lg:px-10 lg:place-self-start lg:mt-10">Lihat
                    Semua Menu</button>
            </a>
        </div>
    </section>
    ...
    <section id="order-guide-section" class="container px-4">
        <div class="order-guide-wrapper p-6 mt-6 lg:mt-32 bg-white rounded-xl flex flex-col gap-4">
            <header
                class="header-order-guide h-56 relative flex items-center justify-start bg-tertiary rounded-xl lg:rounded-3xl overflow-hidden">
                {{-- pattern svg --}}
                <img src="../../images/pattern-for-header-order-guide.svg" alt="pattern svg"
                    class="absolute top-1/2 left-1/2  -translate-x-1/4 -translate-y-1/4 w-full h-full object-cover opacity-5">
                <img src="../../images/people-confused.svg" alt="people confused img"
                    class="w-64 lg:w-80 aspect-square translate-y-5 lg:ms-16 -translate-x-5">
                <div class="text-group flex flex-col justify-center items-start gap-3 -ms-6 text-primary">
                    <h2 class="text-3xl lg:text-4xl">Gimana cara saya <span
            ...
            <div
                class="order-guide-content group flex gap-2 text-secondary/40 font-semibold hover:text-secondary duration-200 cursor-default">
                <div class="order-number relative size-28 flex-1 grid place-content-end text-primary bg-tertiary-100 text-5xl lg:text-8xl rounded-md lg:rounded-2xl overflow-hidden hover:text-primary hover:-translate-y-[2px] hover:shadow-lg hover:shadow-slate-700/10 duration-150 active:translate-y-[1px] cursor-pointer"
                    data-index="1">
                    <span class="absolute -bottom-3 -right-2 lg:-bottom-6 lg:-right-4">
                        01
                    </span>
                </div>
                <div class="order-number relative size-28 flex-1 grid place-content-end bg-tertiary-100 text-5xl lg:text-8xl rounded-md lg:rounded-2xl overflow-hidden hover:text-primary hover:-translate-y-[2px] hover:shadow-lg hover:shadow-slate-700/10 duration-150 active:translate-y-[1px] cursor-pointer"
                    data-index="2">
                    <span class="absolute -bottom-3 -right-2 lg:-bottom-6 lg:-right-4">
                        02
                    </span>
            ...
            <div class="main-content-order-guide w-full bg-tertiary text-primary rounded-xl lg:rounded-3xl p-6">
                <div class="content p-2 rounded-xl lg:flex lg:items-center lg:justify-between">
                    ...
                    <img src="../../images/sign-up.svg" alt="two people discuss svg"
                        class="order-guide-img max-w-[25rem]">
    ...
    <section id="rating-users-section" class="container px-4 lg:mt-52">
        <div class="rating-wrapper w-full overflow-hidden p-6 mt-6 rounded-xl bg-white">
            ...
            <div class="head-content hidden lg:flex text-center flex-col gap-4 mb-16">
                ...
                <p class="text-secondary">Kepuasan pelanggan adalah prioritas kami. Berikut beberapa testimoni dari mereka.</p>
            </div>
            @if ($ulasan->isEmpty())
            ...
            @else
                <div class="card-wrapper flex flex-col lg:grid lg:grid-cols-2 gap-4 mt-8 cursor-default">
                    @foreach ($ulasan as $item)
                        <div class="card rating-card group text-primary flex flex-row p-4 gap-4 bg-tertiary rounded-xl hover:bg-primary duration-300 hover:text-white hover:shadow-lg hover:shadow-slate-400">
                            ...
                            <div class="text-wrapper flex flex-col gap-2">
                                ...
                                <p
                                    class="message-rating text-xs lg:text-sm leading-5 lg:leading-6 text-primary/80 line-clamp-3 group-hover:text-white/80">
                                    {{ $item->pesan }}</p>
                                <p class="rating-created-at text-xs text-secondary/50 mt-3 text-right group-hover:text-white/70">
                                    {{ $item->formatted_date }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
