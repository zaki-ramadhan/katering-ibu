...
@section('content')
    {{-- hero-section --}}
    <section id="hero-section" class="container relative px-4 text-white">
        <div
            ...
            <div class="absolute w-full h-full overlay bg-black/30 top-1/2 left-1/2 -translate-x-1/4 -translate-y-1/4">
            </div>
        </div>
        <div
            class="absolute flex flex-col items-center content-center gap-4 text-center text-button-group top-1/2 left-1/2 -translate-x-1/4 -translate-y-1/4">
            <h1 class="text-2xl md:text-5xlfont-semibold lg:text-7xl w-max">Buat hidangan acaramu<br>jadi lebih berkualitas.</h1>
            <p class="-mt-2 text-sm lg:text-lg lg:mt-2">Katering Ibu - Solusi Kebutuhan Katering Anda.</p>
            @auth
                <a href="{{ route('menu') }}">
                    <button id="btn-order-now"
                        ...
                        <iconify-icon icon="akar-icons:shopping-bag"
                            class="p-2 text-lg duration-200 rounded-full lg:text-2xl bg-primary-600 group-hover:bg-primary"></iconify-icon>
                        {{ auth()->check() ? 'Pesan sekarang' : 'Buat Pesanan Pertama Anda' }}
                        <iconify-icon icon="fluent:arrow-right-20-filled"
                            class="text-lg duration-200 lg:text-xl group-hover:ms-6"></iconify-icon>
                    </button>
                </a>
            @else
                <a href="{{ route('login') }}">
                    <button id="btn-order-now"
                        class="flex items-center content-center gap-2 py-2 mt-3 text-xs duration-200 rounded-full group ps-2 pe-4 bg-primary hover:bg-gradient-to-r hover:from-primaryHovered hover:to-primary lg:text-sm text-secondary hover:text-white active:bg-primary">
                        <iconify-icon icon="akar-icons:shopping-bag"
                            class="p-2 text-lg duration-200 rounded-full bg-primary-600 group-hover:bg-primary"></iconify-icon>
                        Ayo mulai memesan
                        <iconify-icon icon="fluent:arrow-right-20-filled"
                            class="text-lg duration-200 group-hover:ms-6"></iconify-icon>
                    </button>
                </a>
            @endauth
        </div>
    ...
    <section id="features-section" class="container flex items-center justify-center lg:h-[26rem] px-4 mt-6">
        <div class="container px-6 pt-6 bg-white cursor-default features-wrapper lg:bg-transparent lg:pt-20 rounded-xl">
            <h1
            ...
            <div
                class="flex gap-2 py-10 text-sm text-center content-wrapper lg:text-left text-primary hover:text-secondary lg:gap-5">
                <div
    ...
    <section id="top-menu-section" class="container px-4">
        <div class="w-full p-6 mt-6 bg-white top-menu-wrapper md:bg-transparent rounded-xl">
            <h2
            ...
            <div
                class="grid grid-cols-2 gap-4 pt-6 card-wrapper group md:grid-cols-3 lg:grid-cols-4 text-primary hover:text-secondary ">
                @foreach ($bestSellingMenus as $item)
                    <figure
                        class="relative flex-1 p-3 duration-150 border border-transparent card hover:text-primary rounded-2xl hover:border-slate-300">
                        <div class="overflow-hidden img-container aspect-4/3 rounded-2xl">
                            ...
                            {{-- <div class="absolute flex items-center content-center gap-1 px-3 py-1 text-sm font-medium bg-white rounded-full rating-menu top-6 left-5">
                        ...
                        <figcaption class="flex flex-col gap-1 mt-4 card-content">
                            <div class="relative flex items-start justify-between w-full gap-3 text-wrapper">
                                <div class="flex flex-col gap-2 text-primary">
                                    <h3 class="font-normal menu-name line-clamp-1">{{ $item->nama_menu }}</h3>
                            ...
                            @auth
                                <a href="{{ route('order-now.show', ['order_now' => $item->id]) }}" class="w-full">
                                    <button
                                        class="flex items-center justify-center w-full gap-1 py-3 mt-4 text-xs text-white duration-150 rounded-lg btn-order bg-orderDeactive hover:bg-orderHovered active:bg-orderClicked lg:text-sm">
                            ...
                            @else
                                <a href="{{ route('login') }}" class="w-full">
                                    <button
                                        class="flex items-center justify-center w-full gap-1 py-3 mt-4 text-xs text-white duration-150 rounded-lg cursor-default btn-order bg-secondary-300 hover:bg-secondary active:bg-tertairy lg:text-sm">
                                        <iconify-icon icon="tdesign:shop-filled"
                                            class="text-base -translate-y-[1px]"></iconify-icon>
                                        Pesan Sekarang
                                    </button>
    ...
    <section id="menu-section" class="container px-4">
        <div class="w-full px-6 py-6 mt-6 bg-white menu-wrapper lg:mt-16 rounded-xl">
            <h2
                class="lg:hidden text-md text-primary ps-4 relative before:content-[''] before:absolute before:top-1/2 before:left-0 before:-translate-y-[25%] before:bg-primary before:w-1 before:h-full">
            ...
            <div
                ...
                @foreach ($menu as $item)
                    <figure
                        class="relative flex-1 p-3 pb-4 duration-150 border border-transparent card hover:text-primary rounded-2xl hover:border-slate-300">
                        <div class="overflow-hidden img-container aspect-4/3 rounded-2xl">
                            <img src="{{ Storage::url($item->foto_menu) }}" alt="Foto {{ $item->nama_menu }}"
                                class="object-cover w-full h-full duration-200 brightness-100">
                            {{-- <div class="absolute flex items-center content-center gap-1 px-3 py-1 text-sm font-medium bg-white rounded-full rating-menu top-6 left-5">
                        ...
                        <figcaption class="flex flex-col gap-1 mt-4 card-content">
                            <h3 class="font-normal menu-name text-md">{{ $item->nama_menu }}</h3>
                            <p class="menu-price font-bold text-lg before:content-['Rp']">
                                {{ number_format($item->harga, 0, ',', '.') }}</p>
                            @auth
                                <a href="{{ route('order-now.show', ['order_now' => $item->id]) }}" class="w-full">
    ...
    <section id="order-guide-section" class="container px-4">
        <div class="flex flex-col gap-4 p-6 mt-6 bg-white order-guide-wrapper lg:mt-32 rounded-xl">
            <header
                class="relative flex items-center justify-start h-56 overflow-hidden header-order-guide bg-tertiary rounded-xl lg:rounded-3xl">
                {{-- pattern svg --}}
                ...
                <img src="../../images/people-confused.svg" alt="people confused img"
                    class="w-64 -translate-x-5 translate-y-5 lg:w-80 aspect-square lg:ms-16">
                <div class="flex flex-col items-start justify-center gap-3 text-group -ms-6 text-primary">
            ...
            <div
                class="flex gap-2 font-semibold duration-200 cursor-default order-guide-content group text-secondary/40 hover:text-secondary">
    ...
    <section id="rating-users-section" class="container px-4 lg:mt-52">
        <div class="w-full p-6 mt-6 overflow-hidden bg-white rating-wrapper rounded-xl">
            <h2
            ...
            @else
                <div class="flex flex-col gap-4 mt-8 cursor-default card-wrapper lg:grid lg:grid-cols-2">
                    @foreach ($ulasan as $item)
                        <div class="flex flex-row gap-4 p-4 duration-300 card rating-card group text-primary bg-tertiary rounded-xl hover:bg-primary hover:text-white hover:shadow-lg hover:shadow-slate-400">
                            ...
                            <div class="flex flex-col gap-2 text-wrapper">
                                ...
                                {{-- <p class="text-xs email-user lg:text-sm text-secondary/50">{{ str_replace(substr($item->user->email, 1, 8), '******', $item->user->email) }}</p> --}}
                                <p
